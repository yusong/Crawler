(function() {
	...
	var createSaver = function(dis) {
		...
		for( var source in dis.colmap ) {
			for( var cat in dis.colmap[source] ) {
				dispatchers[source + '_' + dis.colmap[source][cat]] = new Saver({ db: source, coll: dis.colmap[source][cat] });
			}
		}
	};
	 ...
	 */
	var getColl = function( source, catStr ) {
		if( Dispatcher.colmap[source][catStr] !== undefined ) {
			return source + '_' + Dispatcher.colmap[source][catStr];
		}
		else {
			return source;
		}
